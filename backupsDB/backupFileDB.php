...
$host="localhost";
$db="db_sanatorio";
$backup_file="backupFile_".$db.'_'.date('d-m-Y').".sql"; //Este es el backup_file del archivo a generar
/* Determina si la tabla será vaciada (si existe) cuando  restauremos la tabla. */ 
date_default_timezone_set('America/Mexico_City');          
$drop = false;
$tablas = false; //tablas de la db
// Tipo de compresion.
...
if ( !headers_sent() ) {
    ...
    switch ($compresion) {
    case "gz":
        header("Content-Disposition: attachment; filename=$backup_file.gz");
        header("Content-type: application/x-gzip");
        echo gzencode($dump, 9);
        break;
    case "bz2": 
        header("Content-Disposition: attachment; filename=$backup_file.bz2");
